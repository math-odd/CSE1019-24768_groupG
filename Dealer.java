//abstract class면 내가 생성을 못한다. 그리고, 
public class Dealer{
...
/* 딜러가 플레이어에게 카드를 먹이는  함수 : dealWantTo, dealTo*/
	...
	//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!여기 문제생김 >> CardPlayerBehavior안 쓰거나 CardPlayer가 상속포기하게 dealer가 만들게 해야함
	public void dealWantTo(CardPlayer p){
	    p.receivedCard(deck.newCard());
		//if(p.wantACard())
	}

	//없어서 먹어야하는 경우 && 공격받아서 먹어야하는 경우  :: n장의 카드를 플레이어 p의 덱에 추가한다. 
	public void dealTo(CardPlayer p ,int n ){ //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!여기 문제생김 위에랑 같음
	    for (int i=0; i<n; i++)
	    	p.receivedCard(deck.newCard());
	}
